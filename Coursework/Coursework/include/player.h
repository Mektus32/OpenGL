...
public:
	...
	void Draw(const glm::mat4& view, const glm::mat4& projection, const Camera& camera, float h_a, float v_a);
}; 
...
void Player::Draw(const glm::mat4& view, const glm::mat4& projection, const Camera& camera, float h_a, float v_a) {
	shader.use();
	...
	model = glm::rotate(model, glm::radians(h_a), camera.Front);
	model = glm::rotate(model, glm::radians(v_a), result_vec);
